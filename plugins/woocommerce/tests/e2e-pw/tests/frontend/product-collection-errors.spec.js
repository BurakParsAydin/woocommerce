...
const test = baseTest.extend({
	...
	category: async ({ page }, use) => {
		...
		await page.goto('wp-admin/edit-tags.php?taxonomy=product_cat&post_type=product');
		await page.locator('#tag-name').fill(name);
		await page.getByRole('button', { name: 'Add new category' }).click();

		const row = page.locator('table.wp-list-table td.name a.row-title').filter({ hasText: name });
		...
		if (!id) throw new Error(`Could not extract category ID for: ${name}`);
	...
	product: async ({ page, category }, use) => {
		...
		await page.goto('http://localhost:8086/wp-admin/post-new.php?post_type=product', {
		...
		const outOfStockRadio = page.getByRole('radio', { name: 'Out of stock' });
		await outOfStockRadio.waitFor({ timeout: 5000 });
		await outOfStockRadio.check();
		await expect(outOfStockRadio).toBeChecked();

		await page.locator(`input[name="tax_input[product_cat][]"][value="${categoryId}"]`).check();
...
test('displays error notice when adding out-of-stock product from Product Collection block', async ({
	...
	await test.step('Expect error notice to appear', async () => {
		const errorNotice = page.locator('.wc-block-components-notice-banner.is-error');
		await expect(errorNotice).toBeVisible();
		await expect(errorNotice).toContainText(/out of stock|cannot be added/i);
	});
});
